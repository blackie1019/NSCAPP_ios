...
(function ($) {
    ...
    $.mobiscroll.presets.select = function (inst) {
        ...
        var ParkName=$(group).attr('label');//取得optgroup label
        var tmpValue=main[elm.val()];
        if(typeof(ParkName)!="undefined"){
            tmpValue=ParkName+"/" + main[elm.val()];
        }
        var input = $('<input type="text" id="' + id + '"  value="'+tmpValue+ '" class="' + s.inputClass + '" readonly />').insertBefore(elm);
        ...
        inst.setSelectVal = function (d, fill, time) {
            ...
            if (fill) {
                var ParkName=$(group).attr('label');//取得optgroup label
                if(typeof(ParkName)!="undefined"){
                    input.val(ParkName+"/"+main[option]);
                }else{
